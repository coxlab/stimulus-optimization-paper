...
Though not fully matching the performances of deep networks trained on massive quantities of data, it can in fact still be quite competitive for a variety of tasks, especially in the regime where large quantities of training samples are not publicly available. %\eg the LFW-a setting we used \cite{krizhevsky2012imagenet} \cite{pinto2009high, cox2011beyond, viglarge}.
...
In addition to the representations inside deep networks, we also tried to probe the face pair matching decisions made based upon those representations, as exemplified in Fig.~\ref{fig:wface}, which is reported for the first time too to our best knowledge. Though being qualitative, the results still positively confirm the effectiveness of these deep representations as in the complete face pair matching decisions.

{\bf Future work.}
In this work, we only studied shallow networks and the ``simplest'' type of deep networks, two-level networks.
Our results can potentially generalize if viewed as the characterization of ``adding an extra level.''
However, it is indeed highly desirable to extend the study into even deeper networks.
Even though the methods and measures proposed in this work are neutral to the depth of networks, numerical difficulties (\eg evaluation and convergence speed, quality of solutions, \etc) might need to be addressed in very deep networks.
