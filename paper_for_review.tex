...
%to enforce the exploration of a larger extent of the fitness landscape 
...
%(since they might as well be random)
...
\newcommand{\defstars}{In this paper, significance levels follow the definition that *, ** and *** correspond to $p<0.05$, $p<0.01$ and $p<0.001$ respectively, where all $p$-values {are} calculated using permutation tests.}

\footnotetext{\defstars \label{fnote:defstars}}

\begin{figure*}
\begin{center}
\includegraphics[width=0.86\textwidth]{Figs/pic2.jpg} 
\end{center}
...
Color of the boarder of an image indicates the linear explainability of the image, with color map following the definition in Fig.~\ref{fig:methods}.
...
These measures further quantify the significant differences between shallow and deep representations, in addition to the separability and classifiability used in previous studies \cite{donahue2014decaf, zeiler2014visualizing}.
...
The highly structural representations of neurons from deep networks using purely random convolution kernels found in the experiments also explain why such ``random'' deep networks can still have good performances, on top of the theoretical analysis for (one-level) shallow networks \cite{saxe2011random}.
